...
data <- rbind(data_f, data_m) %>%
  ...
  filter(!is.na(race)) %>% 
  mutate(step_1 = as.numeric(step_1), step_2 = as.numeric(step_2)) %>%
  mutate(race = gsub("Black or African American", "Black", .$race)) %>%
  mutate(race = gsub("Hispanic, Latino, or of Spanish Origin", "Hispanic", .$race))
...
write_xlsx(df_outliers,"./ERAS_race_outlier_check_2019-20_LOR.xlsx")
...
pdf("ERAS_race_outlier_check_2019-2020_LOR.pdf")
for (i in 1:num_vars) {
...
pdf("ERAS_race_normality_check_2019-20_LOR.pdf")
for (i in 1:num_vars) {
  print(QQplot_list[[i]])
}
dev.off()
...
# post-hoc test
data %>% tukey_hsd(Analytic ~ race)

model <- aov(Analytic ~ race, data=data)
par(mar = c(3, 6, 3, 3))
